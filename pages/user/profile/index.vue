...
<script>
	...
	export default {
		...
		computed: {
			username () {
				return _.get(this.$route.params, 'username') || _.get(this.$store.state.user, 'username');
			}
		},
		methods: {
			async load () {
		...
		mounted () {		
			if (window.DEBUG) console.log("debug - app.pages.user.profile.index.mounted");
			
			if (!this.content && !this.loading) this.load();
		},
		updated () {			
			if (window.DEBUG) console.log("debug - app.pages.user.profile.index.updated");
